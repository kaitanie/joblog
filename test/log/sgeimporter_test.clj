...
(defn check-field [fname]
  (do
    (fact (str "Field " fname " not nil" (not (nil? (fname protobuf-sge-entry-1))) => true))
    ...
    (if (not (fname #{:qsub_time :start_time :end_time}))
      (fact (str "Types match") (type (fname protobuf-sge-entry-1)) => (type (fname parsed-sge-entry-1)))
      (do
...
(check-field :group)
(check-field :ru_msgrcv)
(check-field :account)
(check-field :ru_nvcsw)
(check-field :cpu)
(check-field :ru_utime)
(check-field :iow)
(check-field :jobnumber)

(fact "qsub_time is an SGE date" (is-sge-date? (:qsub_time protobuf-sge-entry-1)) => true)
(check-field :qsub_time)

(check-field :ru_ixrss)
(check-field :granted_pe)
(check-field :ru_stime)
(check-field :ru_msgsnd)
(check-field :ru_inblock)
(check-field :taskid)

(fact "start_time is an SGE date" (is-sge-date? (:start_time protobuf-sge-entry-1)) => true)
(check-field :start_time)

(check-field :slots)
(check-field :ru_wallclock)
(check-field :ru_nivcsw)
(check-field :ru_idrss)

(fact "end_time is an SGE date" (is-sge-date? (:end_time protobuf-sge-entry-1)) => true)
(check-field :end_time)

(check-field :failed)
(check-field :owner)
(check-field :hostname)
(check-field :exit_status)
(check-field :ru_minflt)
(check-field :ru_oublock)
(check-field :ru_ismrss)
(check-field :ru_isrss)
(check-field :ru_maxrss)
(check-field :maxvmem)
(check-field :ru_majflt)

(fact "Check :mem field type" (type  (:mem protobuf-sge-entry-1)) => (type  (:mem parsed-sge-entry-1)))
(check-field :mem)

(check-field :department)
(check-field :ru_nswap)
(check-field :jobname)
(check-field :io)
(check-field :qname)
(check-field :priority)
(check-field :project)
(check-field :ru_nsignals)
